    ...
    enableDisplayToggle(){
        ...
        displayToggle.addEventListener('click', () => {
            if(this.focus){
                ...
                global.geotab.addin.restrictedDataMode.blur();
            } else {
                displayToggle.innerHTML = 'Blur add-in';
                global.geotab.addin.restrictedDataMode.focus(global.api, global.state);
            }
    ...
    listenToHash(){
        ...
        setInterval(()=>{
            if(window.location.hash !== this.storedHash){
                this.storedHash = window.location.hash;
                if(this.storedHash !== '#' + 'restrictedDataMode'){
                    geotab.addin.restrictedDataMode.blur();
                } else {
                    geotab.addin.restrictedDataMode.focus(global.api, global.state);
                }
            }
        }, 100);
    }
